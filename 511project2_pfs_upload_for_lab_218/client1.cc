...
int main(int argc, char *argv[])
{
  ...
  err_value = c->pfs_create(input_fname, 3);
  if(err_value < 0)
    {
  ...
  nread = c->pfs_read(fdes, (void *)buf, 4*ONEKB,0, &cache_hit);
  printf("Read %d bytes to the file\n", nread);

  //At Client 1
  //Write the first 200 bytes of data from the input file onto pfs_file
  err_value = c->pfs_write(fdes, (void *)buf, 4*ONEKB, 0, &cache_hit);
